...
function Discover({ banList, addToBanList, addToHistory }) {
    ...
    const fetchData = async () => {
        try {
            const response = await fetch(`https://api.thedogapi.com/v1/images/search?limit=7&api_key=${ACCESS_KEY}`);
            ...
            {data && (
                <>
                    ...
                    <div className="buttons">
                        <AttributeButton attribute={data.breeds[0]?.name} addToBanList={addToBanList} />
                        <AttributeButton attribute={data.breeds[0]?.origin} addToBanList={addToBanList} />
                        <AttributeButton attribute={data.breeds[0]?.life_span} addToBanList={addToBanList} />
                        <AttributeButton attribute={data.breeds[0]?.weight?.imperial} label={data.breeds[0]?.weight?.imperial ? `${data.breeds[0]?.weight?.imperial} lbs` : null} addToBanList={addToBanList} />
                        <AttributeButton attribute={data.breeds[0]?.breed_group} addToBanList={addToBanList} />
                    </div>
                    <br />
